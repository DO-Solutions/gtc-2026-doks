...
variable "region" {
  type        = string
  description = "DigitalOcean region for deployment (must have GPU Droplets and Managed NFS)"
}
...
variable "gpu_droplet_size" {
  type        = string
  description = "GPU Droplet size slug (e.g., gpu-h100x1-80gb, gpu-h200x8-1128gb-contracted)"
}
...
variable "gpu_node_count" {
  type        = number
  default     = 0
  description = "Number of GPU nodes in the GPU node pool"
}
